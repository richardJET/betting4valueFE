...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    const foundBlogPage = posts.find(
      (blog) =>
        blog.title
          .toLowerCase()
          .replace(/[^\w\s-]/g, "")
  ...
  return blogPage ? (
    <div className="max-w-3xl mx-auto my-8 px-6 lg:px-8">
      <time dateTime={blogPage.date} className="text-gray-500 text-sm">
        {blogPage.date}
      </time>
      ...
      <div
        className="[&>p]:mb-6 [&>ol]:list-decimal [&>ol]:pl-4 [&>ol>li]:mb-4 [&>ul]:list-disc [&>ul]:pl-4 [&>ul>li]:mb-4 mt-6"
        dangerouslySetInnerHTML={{ __html: blogPage.body }}
      ></div>
    </div>
  ) : (
    <ErrorPage />
